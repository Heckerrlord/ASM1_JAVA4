...
public class DAO_Video extends AbstractDAO<Video>{
	...
	public List<Video> findAll(int pageNumber, int pageSize) {
	 ...
	 public long getTotalViewCount() {
	        EntityManager em = JPAUtils.getEntityManager();
	        try {
	            // Tạo truy vấn JPQL để lấy tổng lượt xem của tất cả video
	            String jpql = "SELECT SUM(v.views) FROM Video v";
	            Query query = em.createQuery(jpql);

	            // Thực hiện truy vấn và lấy kết quả (tổng lượt xem)
	            Long totalViewCount = (Long) query.getSingleResult();
	            return totalViewCount != null ? totalViewCount : 0L; // Trả về 0 nếu tổng là null
	        } finally {
	            em.close();
	        }
	    }
	 public List<Video> findVideosByPage(int startRecord, int endRecord) {
		    EntityManager em = JPAUtils.getEntityManager();
		    try {
		        String jpql = "SELECT v FROM Video v ORDER BY v.id"; 
		        Query query = em.createQuery(jpql)
		                .setFirstResult(startRecord)
		                .setMaxResults(endRecord - startRecord);
		        return query.getResultList();
		    } finally {
		        em.close();
		    }
